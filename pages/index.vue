<template>
  <div>
    <div
      v-if="!loading"
      :class="[{'no-location': !hasLocationData}]">
      <template v-if="!hasLocationData && !loading">
        <div class="container">
          <h1>Welcome,</h1>
...
export default {
  ...
  computed: {
    ...
    gMapPanHeading () {
      ...
      if(this.locationResDataNew) {
        let direction    = this.locationResDataNew.address.street_direction,
          quarterSection = this.locationResDataNew.address.quarter_section;
  ...
  methods: {
    ...
    addressChoice(address) {
      ...
      // alert(this.addressResData && canWeMapAddress);
      // alert(canWeMapAddress);
      if(this.addressResData && canWeMapAddress){
        ...
        if(this.locationResDataNew) {
          this.addressSearchAuto = this.locationResDataNew.address.streetAddress
        } else {
    ...
    addressLookup(address) {
      ...
      .then((res) => {
        if(this.isAddressMapped(res.latitude, res.longitude)) {
          ...
          this.errors.addressRes  = null;
          console.dir(address);
          console.dir('would be right here');
          this.locationLookup();
    ...
    locationLookup() {
      ...
      if(this.addressResData.id) {
        ...
        .then((res) => {
          ...
          this.$router.push({query : { address: this.locationResDataNew.address.streetAddress}});
